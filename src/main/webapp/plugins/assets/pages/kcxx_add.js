        ...
        $.post(contextPath + "/kcxx/edit.do", {
            ...
            if (attachementNames != null && attachementUrls != null) {
                ...
                for (var i = 0; i < names.length; i++) {
                    link += "<div style='margin:5px 20px 5px 10px;float:left'><a style='color:red' href='kcxx/download.do?fileName=" + urls[i] + "'>" + names[i] + "</a></div>";
                }
                $("#attachUrl").html(link);
            }
...
function submitForm(entityID) {
    $.ajax({
        type: "post",
        url: contextPath + "/kcxx/" + (entityID ? "update.do" : "add.do"),
